...
class CROHMELabelGraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # TODO layout gt
        layout = box2layout(objs, boxes, self.nc, img_size=self.img_size)
        return image_64, image_128, image_256, objs, boxes, layout, triples


def box2layout(objs, boxes, nc, img_size=(256, 256)):
    N = boxes.size(0)
    ...
    layout = torch.zeros(nc, *img_size, dtype=torch.float32)
    for i in range(N):
        layout[objs[i], y0[i]: y1[i], x0[i]: x1[i]] += 1
...
if __name__ == '__main__':
    ...
    ds = CROHMELabelGraphDataset(root_dir, names, nc=101)
