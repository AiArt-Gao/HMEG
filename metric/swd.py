...
if __name__ == '__main__':
    ...
    class ImagePathDataset(torch.utils.data.Dataset):
        def __init__(self, tar_dir, transforms=None):
            files = []
    ...
    def get_imgs(src_dir, tar_dir, files):
        src_imgs = []
        tar_imgs = []
        transforms = TF.ToTensor()
        for img in files:
            if os.path.exists(os.path.join(tar_dir, img)):
                src_path = os.path.join(src_dir, img)
                src_img = Image.open(src_path).convert('RGB').resize((256, 256))
                src_imgs.append(transforms(src_img).unsqueeze(dim=0))
                tar_path = os.path.join(tar_dir, img)
                tar_img = Image.open(tar_path).convert('RGB').resize((256, 256))
                tar_imgs.append(transforms(tar_img).unsqueeze(dim=0))
        return torch.cat(src_imgs, dim=0), torch.cat(tar_imgs, dim=0)

    src_dir = '../../datasets/results/原始数据/trainB'

    tars_dir = [
                '../../datasets/results/ours_nosub'
                # '../../datasets/results/ours_2stage',
                # '../../datasets/results/ours',
    ...
    for tar_dir in tars_dir:
        distance = []
        for i in tqdm(range(len(FILES) // batch_size)):
            src, tar = get_imgs(src_dir, tar_dir, FILES[i * batch_size: i * batch_size + batch_size])
            distance.append(swd(src, tar, device='cuda'))
        distance = torch.mean(torch.tensor(distance))
